# MEXC Futures Scan

**Scan timestamp:** 2026-07-15T15:25:46Z
**Primary timeframe:** 1H
**Contracts retrieved:** 771
**Contracts eligible:** 331
**Contracts analyzed:** 327
**Contracts skipped:** 444
**Data freshness:** Valida

## BTC Regime

**BTC_USDT price:** 65,296.10
**BTC regime:** BULLISH
**BTC Regime Score:** 100
**EMA 20 / EMA 50 / EMA 200:** 64,672.45 / 64,117.79 / 63,445.28
**ADX / +DI / -DI:** 34.0186 / 31.4829 / 11.44023
**6H return:** 0.96%
**24H return:** 1.69%
**ATR volatility:** 0.5772735482% | veto=False
**Permitted direction:** LONG

## Best Available Opportunity

Nessuna opportunità qualificata.

## Ranked Qualified Opportunities

Nessun setup qualificato.

## Watchlist

- **AVNT_USDT LONG — 79/100:** manca `price_location_valid, reward_risk_valid`. Trigger richiesto: ritorno entro 0,50 ATR dalla zona 0.09403492132–0.09514797388; poi nuova verifica completa. nuovo riferimento d'ingresso con TP2 strutturale almeno 2R e invalidazione tra 0,30 e 2,50 ATR. Score: {'A_btc_alignment': 20, 'B_candidate_structure': 15, 'C_zone_quality': 15, 'D_closed_candle_confirmation': 14, 'E_momentum_divergence': 4, 'F_volume_open_interest': 5, 'G_relative_performance': 4, 'H_execution_quality': 2}
- **SOL_USDT LONG — 77/100:** manca `price_location_valid, reward_risk_valid`. Trigger richiesto: ritorno entro 0,50 ATR dalla zona 76.80273913–77.47865458; poi nuova verifica completa. nuovo riferimento d'ingresso con TP2 strutturale almeno 2R e invalidazione tra 0,30 e 2,50 ATR. Score: {'A_btc_alignment': 20, 'B_candidate_structure': 15, 'C_zone_quality': 15, 'D_closed_candle_confirmation': 12, 'E_momentum_divergence': 4, 'F_volume_open_interest': 6, 'G_relative_performance': 0, 'H_execution_quality': 5}
- **HEMI_USDT LONG — 76/100:** manca `score_below_80`. Trigger richiesto: almeno 4 punti aggiuntivi mantenendo tutti i requisiti: volume di conferma almeno 1,20x la mediana e OI utilizzabile; RSI in rotazione favorevole o divergenza valida; miglioramento della performance relativa contro BTC. Score: {'A_btc_alignment': 20, 'B_candidate_structure': 15, 'C_zone_quality': 18, 'D_closed_candle_confirmation': 12, 'E_momentum_divergence': 4, 'F_volume_open_interest': 3, 'G_relative_performance': 0, 'H_execution_quality': 4}
- **BICO_USDT LONG — 75/100:** manca `price_location_valid`. Trigger richiesto: ritorno entro 0,50 ATR dalla zona 0.01354827057–0.01372172943. Score: {'A_btc_alignment': 20, 'B_candidate_structure': 10, 'C_zone_quality': 15, 'D_closed_candle_confirmation': 12, 'E_momentum_divergence': 4, 'F_volume_open_interest': 7, 'G_relative_performance': 4, 'H_execution_quality': 3}
- **DOT_USDT LONG — 75/100:** manca `price_location_valid`. Trigger richiesto: ritorno entro 0,50 ATR dalla zona 0.8446214724–0.8516880549. Score: {'A_btc_alignment': 20, 'B_candidate_structure': 15, 'C_zone_quality': 15, 'D_closed_candle_confirmation': 12, 'E_momentum_divergence': 4, 'F_volume_open_interest': 6, 'G_relative_performance': 0, 'H_execution_quality': 3}

## Final Decision

**NO QUALIFYING SETUP — STAY FLAT AND WAIT FOR A NEW CLOSED-CANDLE CONFIRMATION.**

> Analisi tecnica automatizzata, non consulenza finanziaria e non istruzione di esecuzione. Nessun ordine viene preparato o inviato.
